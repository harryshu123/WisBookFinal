...
public class SocialNetwork implements SocialNetworkADT {
	...
	// this class is for used to check for the visited or not visited in the dfs part
	...
	public List<String> getShortestPath(Person f1, Person f2,Graph g1) {
     ...
     // Inputs for the DPQ graph 
        ...
        while(!dest.equals(f1.getName())) {
        	//check
        	if(cycle > V) {
        		System.out.println("something wrong!!!  " + cycle);
        		break;
        	}
        	cycle++;
        ...
        for (Map.Entry<String,String> entry : dpq.parent.entrySet())  
...
private	void DFSUtil(int v, List<Vis>b,Graph g1) { 
...
//        System.out.print(v+" "); 
        ...
        for (Person x : g1.getAdjacentVerticesOf(b.get(v).p1)) { 
        	...
        	if(idx!=-1)
            if(!b.get(idx).visited) DFSUtil(idx,b,g1); 
        } 
  
    } 

  private  int connectedComponents(Graph g1) { 
	...
	@Override
	public int getConnectedComponents(Graph g1) {
	
		return connectedComponents(g1);
